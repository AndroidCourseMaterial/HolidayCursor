...
/**
 * Loads the HolidayProvider Content Provide with default holiday data.
 * Static utility class for loading holidays.
 * 
 * @author David Fisher
 *
 */
public class UsNationalHolidays {
	...
	/**
	 * Load values if the content provider is totally empty.
	 * 
	 * @param context
	 */
	...
	/**
	 * Restore the default list of holidays.  Removes all holidays present and loads back in the defaults again.
	 */
	...
	private static void loadHolidayTableWithDefaults() {
		...
		cv.put(HolidayProviderMetaData.HolidayTableMetaData.SAME_DATE_EVERY_YEAR, CONSTANT_DATE);
		...
		cv.put(HolidayProviderMetaData.HolidayTableMetaData.SAME_DATE_EVERY_YEAR, VARIABLE_DATE);
